	...
	private function insertSQL() {
		...
		if(empty($this->error)) {
			...
			$this->query("
				...
				(35, 'site-name', 'Preferred site/product name'),
				(36, 'payment-link', 'Link to checkout'),
				(37, 'support-email', 'Your support email address'),
				(38, 'licensekey', '".$this->options['key']."'),
				(39, 'localkey', ''),
				(40, 'lstatus', '1'),
				(41, 'last-checked', ''),
				(42, 'localkeydays', '1'),
				(43, 'allowcheckfaildays', '6'),
				(44, 'current-version', '2.0');
			", $params);
			...
			$this->query("
				...
				(2, 'a:2:{i:0;s:1:\"2\";i:1;s:1:\"3\";}', 0, 'special', 'Demo Special', 'test.special@co.uk', '0bd6506986ec42e732ffb866d33bb14e'),
				(3, 'a:1:{i:0;s:1:\"3\";}', 0, 'user', 'Demo User', 'test.user@co.uk', 'ee11cbb19052e40b07aac0ca060c23ee');
			", $params);
			...
			if (file_exists('../classes/config.php')) : ?>
				<div class="row">
					<div class="span8">
						...
						<p><span class='label label-important'><?php _e('Important'); ?></span> <?php _e('Please delete or rename the install folder to prevent intrusion'); ?></p>
					</div>
					<div class="span6">
		...
		<form class="form-horizontal" method="post" action="index.php">
			...
			<fieldset>
				...
				<div class="control-group">
					...
					<div class="controls">
						<input type="text" class="input-xlarge" id="dbName" name="dbName" value="<?php if(isset($_POST['dbName'])) echo $_POST['dbName']; else _e('database_name'); ?>">
					</div>
				</div>
				<div class="control-group">
					<label class="control-label" for="dbUser"><?php _e('Username'); ?></label>
					<div class="controls">
						<input type="text" class="input-xlarge" id="dbUser" name="dbUser" value="<?php if(isset($_POST['dbUser'])) echo $_POST['dbUser']; else _e('db username'); ?>">
					</div>
				</div>
				<div class="control-group">
					<label class="control-label" for="dbPass"><?php _e('Password'); ?></label>
					<div class="controls">
						<input type="text" class="input-xlarge" id="dbPass" name="dbPass" value="<?php if(isset($_POST['dbPass'])) echo $_POST['dbPass']; else _e('db password'); ?>">
					</div>
				</div>
			</fieldset>
			...
			<fieldset>
				...
				<div class="control-group">
					...
					<div class="controls">
						<input type="url" class="input-xlarge" id="scriptPath" name="scriptPath" value="<?php if(isset($_POST['scriptPath'])) echo $_POST['scriptPath']; else echo "http://".$_SERVER['HTTP_HOST'].str_replace("install/index.php","",str_replace("functions","",str_replace("\\","/",$_SERVER['SCRIPT_NAME']))); ?>">
				...
				<div class="control-group">
					<label class="control-label" for="email"><?php _e('License key'); ?></label>
					<div class="controls">
						<input type="text" class="input-xlarge" id="key" name="key" value="<?php if(isset($_POST['key'])) echo $_POST['key']; else echo ''; ?>" readonly>
						<p class="help-block"><?php _e('This is your product license key'); ?></p>
					</div>
				</div>
			</fieldset>
			...
			<fieldset>
				...
				<div class="control-group">
					...
					<div class="controls">
						<input type="text" class="input-xlarge" id="adminUser" name="adminUser" value="<?php if(isset($_POST['adminUser'])) echo $_POST['adminUser']; else _e('admin'); ?>">
					</div>
				</div>
				<div class="control-group">
					<label class="control-label" for="adminPass"><?php _e('Password'); ?></label>
					<div class="controls">
						<input type="text" class="input-xlarge" id="adminPass" name="adminPass" value="<?php if(isset($_POST['adminPass'])) echo $_POST['adminPass']; else _e('admin'); ?>">
					</div>
				</div>
			</fieldset>
