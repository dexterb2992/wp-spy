...
class Check extends Generic {
	...
	private function isGuest($forceLogin) {
		...
		if ( !$forceLogin )
			return empty( $_SESSION['vd']['user_id'] );
		...
		if ( empty($_SESSION['vd']['user_id']) ) :
			...
			$_SESSION['vd']['referer'] = $_SERVER['REQUEST_URI'];
	...
	private function isRestricted() {
		...
		if ( !empty($_SESSION['vd']['restricted']) || !empty($_SESSION['vd']['level_disabled']) ) :
	...
	private function forcePwUpdate() {
		...
		if ( !empty($_SESSION['vd']['forcePwUpdate']) && basename($_SERVER['PHP_SELF']) != 'profile.php') :
	...
	public function protectPage($level) {
		...
		*/
		...
		if( ! @array_intersect($level, $_SESSION['vd']['user_level']) && $level != array('*') )
			$this->deny_access();
	...
	public function protectThis($level) {
		...
		*/
		...
		if ( empty( $_SESSION['vd']['user_id'] ) && $level == array('*') )
			return false;
