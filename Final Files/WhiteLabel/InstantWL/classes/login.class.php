...
class Login extends jigowatt_integration {
	...
	function __construct() {
		...
		if ( parent::getOption('signin-redirect-referrer-enable') )
			$_SESSION['vd']['referer'] = (!empty($_SESSION['vd']['referer'])) ? $_SESSION['vd']['referer'] : 'home.php';
		else
			$_SESSION['vd']['referer'] = parent::getOption('signin-redirect-url');
	...
	private function social_login($provider) {
		...
		$params = array( ':session' => $_SESSION['vd'][$provider] );
	...
	private function validate() {
		...
		if(!parent::validatePassword($this->pass, $this->result['password'])) {
			...
			$username = $this->username_type;
			$this->error = "<div class=\"alert alert-error\">" . sprintf(_('Incorrect %s or password.'), $username) . "</div>";
	...
	public function login() {
		...
		if ( parent::getOption('user-activation-enable') ) :
			...
			if ($count > 0) $_SESSION['vd']['activate'] = 1;

		endif;

		// Save if user is restricted
		if ( !empty($this->result['restricted']) ) $_SESSION['vd']['restricted'] = 1;
		...
		$_SESSION['vd']['user_level'] = $user_level;

		$_SESSION['vd']['email'] = $this->result['email'];

		$_SESSION['vd']['gravatar'] = parent::get_gravatar($this->result['email'], true, 26);
		...
		if ( !empty($disRow['level_disabled']) ) $_SESSION['vd']['level_disabled'] = 1;
		...
		$username = $this->username_type;
		$_SESSION['vd']['username'] = $this->result[$username];

		// User ID of the logging in user
		$_SESSION['vd']['user_id'] = $this->result['user_id'];

		if ( empty($redirect) ) $redirect = $_SESSION['vd']['referer'];

		unset(
			$_SESSION['vd']['referer'],
			$_SESSION['vd']['token'],
			$_SESSION['vd']['facebookMisc'],
			$_SESSION['vd']['twitterMisc'],
			$_SESSION['vd']['openIDMisc']
		);
