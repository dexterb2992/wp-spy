...
jQuery(document).ready(function(){
				...
				if( page == "site-info" ){
							...
							// prepare variable to handle data which will be saved to database
								...
								$.ajax({
									...
									if( data.robot == true  || data.robot == "true" || data.robot == "1"){
										...
										$("#robots").addClass("icon-check").html("");
									}else{
										$("#robots").html("N/A");
									}
									...
									if( data.sitemap_index == true || data.sitemap_index == "true" || data.sitemap_index == "1"){
										...
										$("#sitemap").addClass("icon-check").html("");
									}else{
										$("#sitemap").html("N/A");
									}
					...
					$.ajax({
						...
						if( data == false ){
							...
							$("#google_page_rank").html(data.google_page_rank+"/10");
							$.each(data, function (i, row){
								// pages indexed
							...
							if( i !== "alexa_rank_in_country" ){
								...
								if( typeof(row) === 'null' || row === null || row === undefined || row.length === 0 ){
									$(".rank-in-country tbody").append('<tr>'+
										'<td colspan="4">'+
											'<span class="failed">No data available for now. Check out the <a href="'+$("#nav_seo_stats").data("href")+'" target="_blank">SEO Stats</a>  for '+domain_raw+' and try again.</span>'+
										'</td>'+
									'</tr>');
								}else{
	...
	// JAVASCRIPT FUNCTIONS ************************************************************************ 
		...
		function fetching_failure(div, title){
			div.append('<div class="erro red">'+title+' failed to load. Please try again.</div>').delay(4000).fadeOut('slow');
		}
