...
function wpspy_admin_menu() {
  ...
  /* Add our plugin menu and administration screen */
  	$resrow = get_option( 'pspy_access_token' );	
	if($resrow!='') 
...
function wpspy_manage_menu() {
	...
	if($_REQUEST['page']!="wpspy-keycheck") {
			include('wpspy-dashboard.php');                                // The slug to use in the URL of the screen
		 }else{
			include('wpspy-keycheck.php');      
		 }
...
add_action( 'wp_ajax_wp_pspy_checkey', 'wp_pspy_checkey' );
add_action( 'wp_ajax_nopriv_wp_pspy_checkey', 'wp_pspy_checkey' );

function wp_pspy_checkey() 
{
	...
	if($p_type!=''){		
		add_option("pspy_access_token", $p_type);
		echo 'do';die();	
	}	
		//return $success;						
	}
...
function wpspy_uninstall_plugin(){
	...
	delete_option('pspy_access_token');
