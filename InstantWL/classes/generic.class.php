...
class Generic extends Connect {
	...
	public function denyAccessLogs() {
		...
		return ( ($this->getOption('profile-timestamps-admin-enable') && !in_array(1, $_SESSION['vd']['user_level'])) || !$this->getOption('profile-timestamps-enable') );
	...
	public function generateToken() {
		...
		if(empty($_SESSION['vd']['token']))
			$_SESSION['vd']['token'] = md5(uniqid(mt_rand(),true));
	...
	public function definePaths() {
		...
		if (!defined('phplogin_version'))       define( 'phplogin_version',    3.21);
	...
	public function validater() {
			$dbh = parent::$dbh;
		    if ( empty($dbh) ) return false;
			$sql = "SELECT `id`,`option_name`,`option_value` FROM `login_settings` WHERE `option_name` = ? OR `option_name` = ? OR `option_name` = ? OR `option_name` = ? ORDER BY `id` ASC LIMIT 0,4;";
		    $query = $dbh->prepare($sql);
			$array = array('licensekey', 'localkey','lstatus','localkeydays');
			$query->execute($array);
			$result=$query->fetchAll(PDO::FETCH_ASSOC);
			foreach($result as $i=>$res)
			{
			   if($res['option_name']=='licensekey')
			   {
			      $key=$res['option_value'];
			   }
			   else if($res['option_name']=='localkey')
			   {
					$lkey=$res['option_value'];
			   } else if($res['option_name']=='lstatus')
			   {
					$lstatus=$res['option_value'];
			   }else if($res['option_name']=='localkeydays')
			   {
					$lkeydays=$res['option_value'];
			   }
			}
			if($lstatus==='0')
			{
				...
				{
					 $result= parent::checkLicense($_POST['licensekey']);
					 //var_dump($result);
					if($result['status']=='Active')
					{
					 ...
					 $query = $dbh->prepare($sql);
					 $query->bindValue(":name", 'licensekey');
					 $query->bindValue(":value", $_POST['licensekey']);
					 $query->execute();
					 $sql = "UPDATE `login_settings` SET `option_value` = :value WHERE `option_name` = :name";
					 $query = $dbh->prepare($sql);
					 $query->bindValue(":name", 'localkey');
					 $query->bindValue(":value", $result['localkey']);
					 $query->execute();
					 $sql = "UPDATE `login_settings` SET `option_value` = :value WHERE `option_name` = :name";
					 $query = $dbh->prepare($sql);
					 $query->bindValue(":name", 'lstatus');
					 $query->bindValue(":value", '1');
					 $query->execute();
					 $sql = "UPDATE `login_settings` SET `option_value` = :value WHERE `option_name` = :name";
					 $query = $dbh->prepare($sql);
					 $query->bindValue(":name", 'last-checked');
					 $query->bindValue(":value", date("Ymd",mktime(0,0,0,date("m"),date("d"),date("Y"))));
					 $query->execute();
			...
			{
			     $sql = "SELECT `option_value` FROM `login_settings` WHERE `option_name` = ?;";
				$query = $dbh->prepare($sql);
				$array = array('last-checked');
				$query->execute($array);
				$result=$query->fetchAll(PDO::FETCH_ASSOC);
				$lastcheckeddate=$result[0]['option_value'];
				$expdate = date("Ymd",mktime(0,0,0,date("m"),date("d")-$lkeydays,date("Y")));
				
				if($lastcheckeddate>$expdate){ return 0;} //check once a day
			
				$result= parent::checkLicense($key,$lkey);
				//var_dump($result);
				if($result['status']!='Active')
				{
				  $sql = "UPDATE `login_settings` SET `option_value` = :value WHERE `option_name` = :name";
				...
				if($result["remotecheck"])
				{
					 ...
					 $sql = "UPDATE `login_settings` SET `option_value` = :value WHERE `option_name` = :name";
					 $query = $dbh->prepare($sql);
					 $query->bindValue(":name", 'localkeydays');
					 $query->bindValue(":value", $result['localkeydays']);
					 $query->execute();
					 
					 $sql = "UPDATE `login_settings` SET `option_value` = :value WHERE `option_name` = :name";
					 $query = $dbh->prepare($sql);
					 $query->bindValue(":name", 'allowcheckfaildays');
					 $query->bindValue(":value", $result['allowcheckfaildays']);
					 $query->execute();
				}
				 $sql = "UPDATE `login_settings` SET `option_value` = :value WHERE `option_name` = :name";
				 $query = $dbh->prepare($sql);
