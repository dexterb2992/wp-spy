...
class Jigowatt_integration extends Generic {
	...
	private function retrieveInfo() {
		...
		$params = array( ':user_id' => $_SESSION['vd']['user_id'] );
	...
	private function connect($link) {
		...
		$params = array(
			':user_id'      => $_SESSION['vd']['user_id'],
			':session_link' => $_SESSION['vd'][$link]
		);
			...
			if(!$openid->mode) {
				...
				if ($openid->validate()) :
					...
					if( !empty($providers[$link]) ) :
						...
						$_SESSION['vd'][$link] = $openid->identity;
						$this->connect($link);
						$_SESSION['vd']['openIDMisc'] = $openid->getAttributes();
						$_SESSION['vd']['openIDMisc']['type'] = $link;
						$_SESSION['vd']['openIDMisc'][$link] = $openid->identity;
	...
	public function facebook_url() {
		...
		if ($this->fbUser) :
		  ...
		  try {
			$_SESSION['vd']['facebookMisc'] = $user_profile;
			$_SESSION['vd']['facebook'] = $this->fbUser;
	...
	public function twitter_url() {
		...
		if( !empty($_GET['oauth_token']) ) {
			...
			$_SESSION['vd']['twitter'] = $twitterInfo['id'];
			$_SESSION['vd']['twitterMisc']['username'] = $twitterInfo['screen_name'];
			$_SESSION['vd']['twitterMisc']['id'] = $twitterInfo['id'];
			$_SESSION['vd']['twitterMisc']['name'] = $twitterInfo['name'];

		}

		return !empty($_SESSION['vd']['twitter']) ? '#' : $this->twitterObj->getAuthenticateUrl( null,array('oauth_callback' => str_replace("www.","", SITE_PATH) . basename($_SERVER['PHP_SELF']) . '?link=twitter' ));
