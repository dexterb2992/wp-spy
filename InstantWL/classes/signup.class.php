...
class SignUp extends Generic {
	...
	public function getPost($var) {
		...
		if ( !empty($_SESSION['vd']['openIDMisc']) ) :

			$misc = $_SESSION['vd']['openIDMisc'];
		...
		if ( !empty($_SESSION['vd']['facebookMisc']) ) :

			$misc = $_SESSION['vd']['facebookMisc'];
		...
		if ( !empty($_SESSION['vd']['twitterMisc']) ) :

			$misc = $_SESSION['vd']['twitterMisc'];
	...
	private function register() {
		...
		if(empty($this->error)) {
			...
			$_SESSION['vd']['username'] = $this->settings[$this->username_type];

			/* Apply default user_level */
			$_SESSION['vd']['user_level'] = unserialize(parent::getOption('default-level'));

			if ( $requireActivate )
				$_SESSION['vd']['activate'] = 1;

			$_SESSION['vd']['gravatar'] = parent::get_gravatar($this->settings['email'], true, 26);
			...
			$_SESSION['vd']['user_id'] = $user_id;

			/* Social integration. */
			if ( !empty($_SESSION['vd']['facebookMisc']) ) {
				$link = 'facebook';
				$id = $_SESSION['vd']['facebookMisc']['id'];
			}

			if ( !empty($_SESSION['vd']['openIDMisc']) ) {
				$link = $_SESSION['vd']['openIDMisc']['type'];
				$id = $_SESSION['vd']['openIDMisc'][$link];
			}

			if ( !empty($_SESSION['vd']['twitterMisc']) ) {
				$link = 'twitter';
				$id = $_SESSION['vd']['twitterMisc']['id'];
			}
